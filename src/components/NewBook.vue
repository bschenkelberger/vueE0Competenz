<template>
          ...
          <div class="modal-body">
            ...
            <div class="book-input-section">
              ...
              <div v-for="(volume, index) in volumes">
                ...
                <button @click="removeVolumeField(index)" v-show="canRemoveVolume" title="Remove this volume">-</button>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      checkNameErr() {
        return this.name == null || !(this.name.length >= 1 && this.name.length <= 30);
      },
      checkOverviewErr() {
        return this.overview == null || !(this.overview.length >= 10 && this.overview.length <= 100);
      },
      checkVolumesErr() {
        let i = 0;
        while(i < this.volumes.length) {
          if(this.volumes[i].name == null || !(this.volumes[i].name.length >= 1 && this.volumes[i].name.length <= 30))
            return true;
          i++;
        }
        return false;
      ...
      onSubmit() {
        this.errorState = (this.checkNameErr() || this.checkOverviewErr() || this.checkVolumesErr());
        if(!this.errorState) {
          let bookDetails = {
            name: this.name,
